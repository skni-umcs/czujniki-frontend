...
import { arePointsEqual, Event, prepareEventDep, transformPropsToOptions, updateClassNames, useMap } from "maplibre-react-components";
...
export const MyMarker = memo(
    forwardRef<Marker, RMarkerProps>(function RMarker(props, ref) {
        ...
        const [options, callbacks] = transformPropsToOptions(markerProps) as never as [
        ...
        useEffect(() => {
            function onMarkerEvent(e: MarkerEvent) {
                ...
                if (callbacksRef.current?.[callbackName]) {
                    // @ts-expect-error the type of event depends on its nature and
        ...
        if (marker._subpixelPositioning !== subpixelPositioning) {
